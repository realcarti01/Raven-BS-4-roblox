...
local APIControllers = API.Controllers
		...
		if Callback then
			...
			loop:AddTask("KillauraAttack", function(deltaTime)
				...
				if KillauraWallCheck and not isMonster then
					if not APIControllers["SwordController"]:canSee({
						player = target,
				...
				if AnimationOptionsKillaura == 'Normal' and animationdelay <= tick() then
					...
					APIControllers["SwordController"]:playSwordEffect(sword.tool.Name, false)
				end
				...
				local chargedmeta = APIControllers["ItemMeta"][sword.tool.Name]
				if workspace:GetServerTimeNow() - lastAttackTime >= AttackDelay then
					APIControllers["SwordController"].lastAttack = workspace:GetServerTimeNow()
					APIControllers["SwordController"].lastAttackTimeDelta = workspace:GetServerTimeNow() - lastAttackTime
					APIControllers["SwordController"].lastAttackTime = workspace:GetServerTimeNow()
					lastAttackTime = APIControllers["SwordController"].lastAttackTime
					for i = 0, 5, 1 do 
						task.wait()
		...
		if AutoClicker then
			...
			repeat
				...
				if IsAlive(LocalPlayer) then
					if ((APIControllers["KatanaController"] == nil and true) or APIControllers["KatanaController"].chargingMaid == nil) and isNotHoveringOverGui() and AutoClicker and autoclickermousedown then
						if firstClick <= tick() then
							APIControllers["SwordController"]:swingSwordAtMouse()
						else
							firstClick = tick() + 0.1
						end
					end
					if Autoclicker and AutoClickerBlocks and autoclickermousedown and APIControllers["BlockPlacementController"].blockPlacer and firstClick <= tick() then
						if (workspace:GetServerTimeNow() - APIControllers["BlockCpsController"].lastPlaceTimestamp) > (1 / 24) then
							local mouseinfo = APIControllers["BlockPlacementController"].blockPlacer.clientManager
                                :getBlockSelector():getMouseInfo(0)
							if mouseinfo then
								if mouseinfo.placementPosition == mouseinfo.placementPosition then
									APIControllers["BlockPlacementController"].blockPlacer:placeBlock(mouseinfo
                                        .placementPosition)
								end
							end
						end
...
Combat:CreateToggle({
	...
	Callback = function(Callback)
		...
		if EnabledSprint then
			oldSprintFunction = APIControllers["SprintController"].stopSprinting
			APIControllers["SprintController"].stopSprinting = function(...)
				local originalCall = oldSprintFunction(...)
				APIControllers["SprintController"]:startSprinting()
				return originalCall
			end
			LocalPlayer.CharacterAdded:Connect(function(char)
				char:WaitForChild('Humanoid', 9e9)
				task.wait(0.5)
				APIControllers.SprintController:stopSprinting()
			end)
			task.spawn(function()
				APIControllers.SprintController:startSprinting()
			end)
		else
			APIControllers["SprintController"].stopSprinting = oldSprintFunction
			APIControllers["SprintController"]:stopSprinting()
		end
	end
})
...
Velocity = Combat:CreateToggle({
	...
	Callback = function(Callback)
		if Callback then
			applyKnockback = APIControllers["KnockbackUtil"].applyKnockback
			APIControllers["KnockbackUtil"].applyKnockback = function(root, mass, dir, knockback, ...)
			...
			APIControllers["KnockbackUtil"].applyKnockback = applyKnockback
		end
	end
})
...
local function canBuyItem(itemName, shopId)
	...
	local currency = API.Inventory.getItem(item.currency)
			...
			spawn(function()
				while EnabledAutoBuy and task.wait() do
					...
					if EnabledSwordsBuy then
						local currentSword = API.Inventory.getSword()
					...
					if EnabledArmorBuy then
						...
						for i = 1, 4 do
							if API.Inventory.getItem(armors[i]) then
								hasArmor = true
...
LongJumpModule = Blatant:CreateToggle({
	...
	Callback = function(Callback)
		...
		if LongJump then
			...
			if API.Inventory.getItem("tnt") then
			...
			if API.Inventory.getItem("fireball") then
...
if game.PlaceId ~= 6872265039 then
	local oldZephyrUpdate = APIControllers["ZephyrController"].updateJump
	APIControllers["ZephyrController"].updateJump = function(self, orb, ...)
...
task.spawn(function()
	APIControllers["ClientHandler"]:WaitFor("EntityDamageEvent"):andThen(function(v)
...
FOV = Render:CreateToggle({
	...
	Callback = function(Callback)
		...
		if FOVEnabled then
			...
			oldfov = APIControllers["FovController"].setFOV
			oldfov2 = APIControllers["FovController"].getFOV
			APIControllers["FovController"].setFOV = function(self, fov)
				return oldfov(self, FOVValue)
			end
			APIControllers["FovController"].getFOV = function(self, fov)
				return FOVValue
			end
		else
			APIControllers["FovController"].setFOV = oldfov
			APIControllers["FovController"].getFOV = oldfov2
		end
		APIControllers["FovController"]:setFOV(APIControllers["ClientHandlerStore"]:getState().Settings.fov)
	end
})
...
ViewModelChanger = Render:CreateToggle({
	...
	Callback = function(Callback)
		...
		if viewmodel and ViewModelChangerEnabled == true then
			...
			APIControllers.ViewmodelController:setHeldItem(LocalPlayer.Character and
...
FPSBoost = Render:CreateToggle({
	...
	Callback = function(Callback)
		...
		if FPSBoostEnabled then
			fpsboosttextures()
			for i, v in next, (APIControllers["KillEffectController"].killEffects) do
				basetextures[i] = v
				APIControllers["KillEffectController"].killEffects[i] = {
					new = function(char)
						return {
							onKill = function()
							end,
			...
			old = APIControllers["HighlightController"].highlight
			old2 = getmetatable(APIControllers["StopwatchController"]).tweenOutGhost
			getmetatable(APIControllers["StopwatchController"]).tweenOutGhost = function(p17, p18)
				p18:Destroy()
			end
			APIControllers["HighlightController"].highlight = function()
			end
		else
			for i, v in next, (basetextures) do
				APIControllers["KillEffectController"].killEffects[i] = v
			end
			fpsboosttextures()
			debug.setupvalue(APIControllers["KillEffectController"].KnitStart, 2, APIControllers["ClientSyncEvents"])
			APIControllers["HighlightController"].highlight = old
			getmetatable(APIControllers["StopwatchController"]).tweenOutGhost = old2
			old = nil
			old2 = nil
		end
	end
...
Render:CreateToggle({
	...
	Callback = function(Callback)
		...
		if EnabledIndicator then
			debug.getupvalue(APIControllers["DamageIndicator"], 10, {
				Create = old69
			})
			debug.setupvalue(APIControllers["DamageIndicator"], 10, {
...
Render:CreateToggle({
	...
	Callback = function(Callback)
		...
		if CustomSounds then
			...
			for i, v in next, (APIControllers["CombatController"].killSounds) do
				APIControllers["CombatController"].killSounds[i] = oldbedwarssoundtable.KILL
			end
			for i, v in next, (oldbedwarssoundtable) do
				local item = APIControllers["SoundList"][i]
				if item then
					APIControllers["SoundList"][i] = v
				end
			end
		end
	end
...
Utility:CreateToggle({
	...
	Callback = function(Callback)
		if Callback then
			for i, v in pairs(APIControllers["ShopItems"]) do
				tiered[v] = v.tiered
				...
				local success, loopResult = pcall(function()
					...
					if IsAlive(LocalPlayer) then
						...
						for i, bed in pairs(beds) do
							...
							if mag < BedDistance and APIControllers["BlockEngine"]:isBlockBreakable({
						...
						for i, bed in pairs(beds) do
							...
							if mag < BedDistance and APIControllers["BlockEngine"]:isBlockBreakable({
								...
								if BedNukerAnimation then
									local animation = APIControllers["AnimationUtil"]:playAnimation(LocalPlayer, APIControllers["BlockEngine"]:getAnimationController():getAssetId(1))
									APIControllers.ViewmodelController:playAnimation(15)
									task.wait(0.3)
									animation:Stop()
									animation:Destroy()
								end
	...
	Callback = function(Callback)
		...
		if AutoKitEnabled then
			repeat
				task.wait(0.1)
				if IsAlive(LocalPlayer) then
					infernal = API.Inventory.getItem("infernal_saber") -- not gona put a Kit Equipt check, since this is basically universal :) (with the sword ofc)
					if infernal then
						API.Inventory.equipItem(infernal.tool)
						game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out
					...
					elseif RavenEquippedKit == "grim_reaper" then
						local itemdrops = APIControllers["GrimReaperController"].soulsByPosition --this
						for i, v in pairs(itemdrops) do
...
local function getScaffoldBlock()
	local currentHand = API.Inventory.getInventory(LocalPlayer).items[1]
	if currentHand and APIControllers["ItemMeta"][currentHand.itemType].block then
	...
	elseif not LimitItemEnabled then
		...
		elseif not WoolOnlyEnabled then
			for _, item in ipairs(API.Inventory.getInventory(LocalPlayer).items) do
				if APIControllers["ItemMeta"][item.itemType].block then
				...
				if not block then
					...
					if blockpos then
						if BuildAnimation then
							APIControllers["ViewmodelController"]:playAnimation(APIControllers["AnimationType"].FP_USE_ITEM)
						end
...
Utility:CreateToggle({
	...
	Callback = function(Callback)
		if Callback then
			OldSetSpeedFunc = APIControllers["SprintController"].setSpeed
			APIControllers["SprintController"].setSpeed = function(v, i)
				if LocalPlayer.character.Humanoid then
					LocalPlayer.character.Humanoid.WalkSpeed = math.max(20 * v.moveSpeedMultiplier, 20)
				end
			end
			APIControllers["SprintController"]:setSpeed(20)
		else
			APIControllers["SprintController"].setSpeed = OldSetSpeedFunc
			APIControllers["SprintController"]:setSpeed(20)
			OldSetSpeedFunc = nil
		end
	end
...
Utility:CreateToggle({
	...
	Callback = function(Callback)
		...
		if AntiAFKEnabled then
			repeat
				task.wait(10)
				APIControllers["ClientHandler"]:Get('AfkInfo'):SendToServer({
					afk = false
				})
